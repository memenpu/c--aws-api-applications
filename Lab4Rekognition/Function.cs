...
namespace Lab4Rekognition
{
    public class Function
    {
        ...
        public async Task FunctionHandler(S3Event input, ILambdaContext context)
        {
            foreach (var record in input.Records)
            {
                ...
                ImageMetaData imageMetaData = new ImageMetaData { 
                    ImageUrl = GeneratePreSignedURL(record.S3.Bucket.Name, record.S3.Object.Key) ,
                ...
                foreach(var label in detectResponses.Labels)
                {
                    if(tags.Count < 10)
                    {
                        ...
                        imageMetaData.Labels[label.Name] = label.Confidence;
                    }
                    else
                    {
                ...
                await dynamoDBContext.SaveAsync<ImageMetaData>(imageMetaData);
            }
            return;
        }
        ...
        /// <returns></returns>
        string GeneratePreSignedURL(string bucketName,string objectKey, double duration=1)
        {
            string urlString = "";
            try
            {
                ...
                urlString = this.S3Client.GetPreSignedURL(request1);
            }
